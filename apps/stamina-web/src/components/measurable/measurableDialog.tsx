...
export default function MeasurableDialog() {
  ...
  const { data: measurableToEdit } = useQuery(
    trpc.measurable.findById.queryOptions(measurableIdToEdit, {
      enabled: !!measurableIdToEdit,
    }),
  );

  const createMeasurable = useMutation(
    trpc.measurable.create.mutationOptions({
      onSuccess: async () => {
  ...
  const updateMeasurable = useMutation(
    trpc.measurable.update.mutationOptions({
      onSuccess: async () => {
        await queryClient.invalidateQueries(
          trpc.measurable.findAll.queryFilter(),
        );
  ...
  const validateForm = () => {
    ...
    if (type === MeasurableEnum.Countdown && !dueDate) return false;
  ...
  useEffect(() => {
    if (measurableToEdit) {
      ...
      setDescription("");
      setArea(null);
      setType(MeasurableEnum.Countdown);
  ...
  useEffect(() => {
    if (type !== MeasurableEnum.Countdown) return;
  ...
  return (
    ...
    >
      <DialogContent className="h-3/4 px-0 lg:h-1/2">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {measurableTypes.map((t) => (
                <div
                  ...
                  className={`flex h-24 w-32 flex-col items-center justify-center gap-2 rounded-md select-none ${type === (t.label as MeasurableEnum) ? "border-accent bg-accent/40 border-2" : "border"} p-4`}
                >
                  {t.label}
                  {t.icon}
                </div>
            ...
            <span className="text-muted-foreground h-10 text-sm">
              {
                measurableTypes.find(
                  (mt) => (mt.label as MeasurableEnum) === type,
                )?.description
              }
            </span>
          </div>
          <AnimatePresence initial={false}>
            {type === MeasurableEnum.Countdown && (
              <motion.div
        ...
        <DialogFooter>
          <DialogClose asChild>
            <Button variant="outline" disabled={createMeasurable.isPaused}>
              Cancel
            </Button>
          </DialogClose>
          <Button
            ...
            disabled={
              !validToCreate ||
              createMeasurable.isPending ||
              updateMeasurable.isPending
            }
          >
            {createMeasurable.isPending || updateMeasurable.isPending ? (
              <Spinner />
            ) : measurableIdToEdit ? (
              "Update"
            ) : (
